...
describe("gordo-mcp server", () => {
    ...
    describe("framework.get-metadata", () => {
        it("returns server metadata", () => {
            const result = handleToolCall("framework.get-metadata", {});
        ...
        it("includes usage pattern", () => {
            ...
            assert.ok(result.usage_pattern.includes("get-scaffold"));
        });
    });
    describe("framework.get-introduction", () => {
        it("returns introduction with pitch", () => {
            const result = handleToolCall("framework.get-introduction", {});
            assert.ok(result.pitch);
            assert.ok(result.pitch.length > 100);
        });
        ...
        it("includes more info URL", () => {
            ...
            assert.ok(result.more_info_url.includes("github.com"));
        });
    });
    describe("framework.get-scaffold", () => {
        it("returns hub overview", () => {
            const result = handleToolCall("framework.get-scaffold", {});
    ...
    describe("framework.get-questions", () => {
        it("returns questions array", () => {
            const result = handleToolCall("framework.get-questions", {});
        ...
        it("accepts optional context parameter", () => {
            const result = handleToolCall("framework.get-questions", { context: "software project" });
            assert.ok(result.questions);
        });
    });
    describe("framework.list-documents", () => {
        it("returns documents array", () => {
            const result = handleToolCall("framework.list-documents", {});
        ...
        it("document URIs use gordo:// scheme", () => {
            ...
            const documents = result.documents;
            for (const doc of documents) {
                assert.ok(doc.uri.startsWith("gordo://"), `${doc.id} URI should use gordo:// scheme`);
            }
        });
    ...
    describe("framework.get-document", () => {
        it("retrieves constitution document", () => {
            const result = handleToolCall("framework.get-document", { document: "constitution" });
            assert.ok(result.title);
            assert.ok(result.content);
            assert.ok(result.content.includes("Value"));
        });
        it("retrieves ai-perspective document", () => {
            const result = handleToolCall("framework.get-document", { document: "ai-perspective" });
            assert.ok(result.title);
            assert.ok(result.content);
        });
        it("retrieves foundations document", () => {
            const result = handleToolCall("framework.get-document", { document: "foundations" });
            assert.ok(result.title);
            assert.ok(result.content);
        });
        it("retrieves values document", () => {
            const result = handleToolCall("framework.get-document", { document: "values" });
            assert.ok(result.title);
            assert.ok(result.content);
            assert.ok(result.content.includes("Consent Is Mutual"));
        });
        it("retrieves getting-started document", () => {
            const result = handleToolCall("framework.get-document", { document: "getting-started" });
            assert.ok(result.title);
            assert.ok(result.content);
        });
        it("includes source URL", () => {
            ...
            assert.ok(result.source_url.includes("github.com"));
        });
        ...
        it("returns error for unknown document", () => {
            const result = handleToolCall("framework.get-document", { document: "nonexistent" });
    ...
    describe("framework.list-examples", () => {
        it("returns examples array", () => {
            const result = handleToolCall("framework.list-examples", {});
        ...
        it("example URIs use gordo:// scheme", () => {
            ...
            const examples = result.examples;
            for (const ex of examples) {
                assert.ok(ex.uri.startsWith("gordo://"), `${ex.id} URI should use gordo:// scheme`);
            }
        });
    ...
    describe("framework.get-example", () => {
        it("retrieves ai-guide example", () => {
            const result = handleToolCall("framework.get-example", { example: "ai-guide" });
            assert.ok(result.name);
            assert.ok(result.content);
            assert.ok(result.content.includes("Collaboration Identity"));
        });
        it("retrieves constitution example", () => {
            const result = handleToolCall("framework.get-example", { example: "constitution" });
            assert.ok(result.name);
            assert.ok(result.content);
        });
        it("retrieves session-log example", () => {
            const result = handleToolCall("framework.get-example", { example: "session-log" });
            assert.ok(result.name);
            assert.ok(result.content);
            assert.ok(result.content.includes("Session"));
        });
        it("retrieves handshake example", () => {
            const result = handleToolCall("framework.get-example", { example: "handshake" });
            assert.ok(result.name);
            assert.ok(result.content);
        });
        it("includes guidance", () => {
        ...
        it("includes prerequisites", () => {
            ...
            assert.ok(result.prerequisites.length > 0);
        });
        it("guidance warns about conversation first", () => {
            const result = handleToolCall("framework.get-example", { example: "constitution" });
            assert.ok(result.guidance.includes("STOP"));
        });
        it("returns error for unknown example", () => {
            const result = handleToolCall("framework.get-example", { example: "nonexistent" });
            assert.ok(result.error);
            const error = result.error;
            assert.strictEqual(error.code, "NOT_FOUND");
            assert.ok(error.available);
    ...
    describe("framework.list-primitives", () => {
        it("returns primitives array", () => {
            const result = handleToolCall("framework.list-primitives", {});
    ...
    describe("framework.search", () => {
        it("returns results for consent query", () => {
            const result = handleToolCall("framework.search", { query: "consent" });
            const results = result.results;
            assert.ok(Array.isArray(results));
            assert.ok(results.length > 0);
        });
        it("returns results for trust query", () => {
            const result = handleToolCall("framework.search", { query: "trust earned" });
            const results = result.results;
            assert.ok(Array.isArray(results));
            assert.ok(results.length > 0);
        });
        it("returns results for session query", () => {
            const result = handleToolCall("framework.search", { query: "session continuity" });
            const results = result.results;
            assert.ok(results.length > 0);
        });
        it("each result has required fields", () => {
            const result = handleToolCall("framework.search", { query: "collaboration" });
            const results = result.results;
        ...
        it("respects limit parameter", () => {
            const result = handleToolCall("framework.search", { query: "the", limit: 2 });
            const results = result.results;
            assert.ok(results.length <= 2);
        });
        it("results are sorted by relevance", () => {
            const result = handleToolCall("framework.search", { query: "consent bilateral" });
            const results = result.results;
            if (results.length > 1) {
                for (let i = 0; i < results.length - 1; i++) {
                    assert.ok(results[i].relevance >= results[i + 1].relevance, "results should be sorted by relevance descending");
                }
            }
        });
        it("returns empty results for nonsense query", () => {
            const result = handleToolCall("framework.search", { query: "xyzzy987" });
            const results = result.results;
            assert.ok(Array.isArray(results));
            assert.strictEqual(results.length, 0);
        });
        it("searches both documents and examples", () => {
            const result = handleToolCall("framework.search", { query: "session log" });
            const results = result.results;
            const docTypes = new Set(results.map(r => r.doc_type));
            assert.ok(docTypes.size > 0);
        });
        it("default limit is 5", () => {
            const result = handleToolCall("framework.search", { query: "the and" });
            const results = result.results;
            assert.ok(results.length <= 5);
        });
        it("includes index type in response", () => {
            const result = handleToolCall("framework.search", { query: "trust" });
            assert.strictEqual(result.index_type, "embedded");
        });
    });
    describe("unknown tool handling", () => {
        it("returns error for unknown tool", () => {
            const result = handleToolCall("framework.nonexistent", {});
            assert.ok(result.error);
            const error = result.error;

...
function sendRequest(request: object): Promise<MCPResponse> {
...
describe("MCP Protocol", () => {
  ...
  describe("tools/call", () => {
    it("calls framework.get-metadata", async () => {
      ...
      const text = JSON.parse(content[0].text as string);
...
describe("Content Integrity", () => {
  it("constitution document contains all 8 values", async () => {
    ...
    const result = response.result as Record<string, unknown>;
    const content = result.content as Array<Record<string, unknown>>;
    const doc = JSON.parse(content[0].text as string);
  ...
  it("questions cover key conversation topics", async () => {
    ...
    const topics = data.questions.map((q: {topic: string}) => q.topic);
  ...
  it("scaffold references valid example IDs", async () => {
    ...
    const scaffoldResult = scaffoldResponse.result as Record<string, unknown>;
    const scaffoldContent = scaffoldResult.content as Array<Record<string, unknown>>;
    const scaffold = JSON.parse(scaffoldContent[0].text as string);
    ...
    const examplesResult = examplesResponse.result as Record<string, unknown>;
    const examplesContent = examplesResult.content as Array<Record<string, unknown>>;
    const examples = JSON.parse(examplesContent[0].text as string);
    const validIds = examples.examples.map((e: {id: string}) => e.id);

    for (const component of scaffold.components) {
      assert.ok(validIds.includes(component.example_id),
        `scaffold component ${component.name} references invalid example_id: ${component.example_id}`);
    }
  });
  ...
  it("primitives have valid GitHub URLs", async () => {
    ...
    for (const primitive of data.primitives) {
      assert.ok(primitive.repo_url.startsWith("https://github.com/"),
        `${primitive.id} has invalid repo_url`);
      assert.ok(primitive.repo_url.includes("gordo-"),
        `${primitive.id} repo_url should contain gordo-`);
    }
  });
});
